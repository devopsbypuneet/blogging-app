pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/devopsbypuneet/blogging-app.git'
            }
        }
        stage('Compile') {
            steps {
        ...
        stage('Docker Build & Tag') {
            steps {
                ...
                sh "docker build -t testmail001/gab-blogging-app ."
                }
                }
            }
        }
        ...
        stage('Docker Push Image') {
            steps {
                script{
                withDockerRegistry(credentialsId: 'dockerhub-cred', url: 'https://index.docker.io/v1/') {
                    sh "docker push testmail001/gab-blogging-app"
                }
                }
            }
        }
